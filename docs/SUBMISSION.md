...
UI. A resume PDF goes to Bedrock as a Converse document block — no parsing
library — and comes back as proposed fields the user edits before saving.
...
I rewrote the model layer onto the **Converse API**, which is the cross-model
interface — same request shape for Nova, Llama, Mistral or Claude — and moved to
Amazon Nova, which AWS serves directly rather than brokering. Structured output
...
**A DynamoDB query returns at most 1 MB, and `Select=COUNT` is no exception —
it counts only what that page scanned.** Without following `LastEvaluatedKey`,
